"""
JEPA + Effective Rank Tracking — GPU-Optimised

Same as jepa_model_gpu.py but additionally computes effective rank of encoder
layer activations throughout training, plotted alongside the loss curves.
...
def effective_rank(X, eps=1e-12):
    """Effective rank of activation matrix X (n_samples, features).

    Computes exp(Shannon entropy) of the normalised squared singular values.
    High = diverse, orthogonal features.  Low = collapsed, redundant.
    """
    if X.size(0) < 2:
        return 0.0
...
@torch.no_grad()
def measure_encoder_ranks(model, loader, device, amp_dtype, max_batches=4):
    """Measure effective rank at each encoder layer (target encoder).

    Returns dict mapping layer name → effective rank, computed over
    a few batches of flattened (timestep, d_model) activations.
    """
    raw = model._orig_mod if hasattr(model, '_orig_mod') else model
    raw.eval()

    per_layer = None
    for i, (x, _state, _mask) in enumerate(loader):
        if i >= max_batches:
            break
        x = x.to(device, non_blocking=True)
        with torch.autocast(device_type='cuda', dtype=amp_dtype):
            layer_outs = raw.target_encoder.forward_layers(x)
        # Flatten (batch, seq, d) → (batch*seq, d)
        if per_layer is None:
            per_layer = [[] for _ in range(len(layer_outs))]
        for li, lo in enumerate(layer_outs):
            per_layer[li].append(lo.reshape(-1, lo.size(-1)))
    ...
    for li, chunks in enumerate(per_layer):
        ...
        # Subsample if huge to keep SVD fast
        if cat.size(0) > 20000:
            idx = torch.randperm(cat.size(0), device=cat.device)[:20000]
            cat = cat[idx]
        ranks[f'layer_{li+1}'] = effective_rank(cat)
    return ranks
...
def plot_loss_curve(train_losses, val_losses, cos_sims, lrs=None,
                    rank_history=None, save_path='training_loss.png'):
    """Training/validation loss + cosine similarity + effective rank."""
    n_panels = 2 if rank_history else 1
    ...
    # --- Panel 1: loss + cosine similarity ---
    ax1 = axes[0]
    ...
    ax1r = ax1.twinx()
    ax1r.plot(epochs, cos_sims, color='green', linewidth=1.5, alpha=0.7,
              label='Cosine Similarity')
    ax1r.set_ylabel('Cosine Similarity', color='green')
    ...
    if rank_history:
        ax2 = axes[1]
        rank_epochs = sorted(rank_history.keys())
        layer_names = sorted(rank_history[rank_epochs[0]].keys(),
                             key=lambda s: int(s.split('_')[1]))
        cmap = plt.cm.viridis
        for i, ln in enumerate(layer_names):
            colour = cmap(i / max(1, len(layer_names) - 1))
            vals = [rank_history[e][ln] for e in rank_epochs]
            ax2.plot(rank_epochs, vals, label=ln.replace('_', ' ').title(),
                     linewidth=1.5, color=colour)
        ax2.set_xlabel('Epoch')
        ax2.set_ylabel('Effective Rank')
        ax2.set_title('Effective Rank of Target Encoder Layers')
...
def main():
    ...
    rank_history = {}  # epoch → {layer_name: effective_rank}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % rank_every == 0 or epoch == 1 or epoch == args.epochs:
            ranks = measure_encoder_ranks(
                model, val_loader, device, amp_dtype)
            rank_history[epoch] = ranks
            rank_str = '  '.join(f'L{k.split("_")[1]}={v:.1f}'
                                 for k, v in sorted(ranks.items()))
            print(f"  Effective rank: {rank_str}")
